function [input, pls_opts, save_opts] = y_pls_input(X0, Y0behav, diagnosis, outputDir, Nperm, Nboots)
...
% NOTE: this script is modified based on pls_input.m in myPLS toolbox
% by Yongbin Wei, Ph.D., @VU Amsterdam 2021
...
input.group_names={'HC', 'SZ'}; 

% --- Names of the behavior data ---
% input.behav_names = {'SCZ', 'BD', 'SCZ+BD', 'SCZ-BD'}; 

% --- Names of the imaging data ---
% will only be used if save_opts.img_type='barPlot'
for ii = 1:size(X0,2); input.img_names{ii,1} = ['c ' num2str(ii)]; end
...
pls_opts.nPerms = Nperm;
pls_opts.nBootstraps = Nboots;
...
save_opts.output_path = outputDir;
...
% --- Bar plot options ---
save_opts.plot_boot_samples = 0; % binary variable indicating if bootstrap samples should be plotted in bar plots
save_opts.errorbar_mode = 'std'; % 'std' = plotting standard deviations; 'CI' = plotting 95% confidence intervals
save_opts.hl_stable = 0; % binary variable indicating if stable bootstrap scores should be highlighted
